...
if ( ! class_exists( 'DB_Reset_Admin' ) ) :
	...
	class DB_Reset_Admin {
		...
		public function reset() {
			if ( $this->form_is_safe_to_submit() ) {
				try {
					...
					$this->resetter->set_upload_delete( $this->request['db-reset-delete-uploads'] );
		...
		private function handle_after_reset() {
			...
			$this->notice_success = __( 'The selected tables were reset', 'wordpress-site-reset' );
		}
		...
		private function assert_correct_code() {
			if ( $this->request['db-reset-code'] !== $this->request['db-reset-code-confirm'] ) {
				$this->notice_error = __( 'You entered the wrong security code', 'wordpress-site-reset' );
				return false;
			}
		...
		public function add_tools_menu() {
			$plugin_page = add_management_page(
				__( 'Site Reset', 'wordpress-site-reset' ),
				__( 'Site Reset', 'wordpress-site-reset' ),
				'manage_network',
				'site-reset',
				array( $this, 'render' )
		...
		private function load_stylesheets() {
			...
			wp_enqueue_style(
				'site-reset',
				plugins_url( 'assets/css/site-reset.css', __FILE__ ),
				array( 'bsmselect' ),
				$this->version
			);
		}
		...
		private function load_javascript() {
			...
			wp_enqueue_script(
				'site-reset',
				plugins_url( 'assets/js/site-reset.js', __FILE__ ),
		...
		private function load_javascript_vars() {
			return array(
				'confirmAlert' => __( 'Are you sure you want to continue?', 'wordpress-site-reset' ),
				'selectTable'  => __( 'Select Tables', 'wordpress-site-reset' ),
			);
		}
